...
multiworld can pick differently. The Fourth Armor has no setting of its own; it
is drawn from X's palette, so `x_palette` recolours it too. If you want to
change a colour without generating a new seed, your own `host.yaml` can
override it when the patch is opened — the setup guide explains how.
