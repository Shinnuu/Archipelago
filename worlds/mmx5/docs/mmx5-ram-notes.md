...
| `0x800D1C47` | u8 | **Max HP — X** (base 0x20; +2 per heart tank, +2 per Alia Life-Up). **Read with a SIGNED `lb`, so 0x7F is the hard ceiling** — 0x80+ reads negative and every bar calculation runs backwards. Vanilla tops out at 0x40 (32 + 8 hearts + 8 Life-Ups, all ×2), which is NOT the engine limit; the apworld's `starting_hp`/`heart_tank_value` use the full 1..0x7F. **THE ENGINE DOES NOT RESPECT ITS OWN CEILING (2026-09-08):** the vanilla Heart Tank collect handler at `0x800540A0` is `lbu` / `addiu v0,v0,2` / `sb` with NO bounds check (disassembled off the disc), and Alia's Life Up reward is the same shape. Vanilla tops out at 0x40 so it never needed one — but landing on top of the apworld's 0x7F clamp they write 0x81, which the life bar reads as −127 and indexes 79 frames off the front of its artwork. That is a live bug report, not a hypothesis: HUD tiles across the screen, player frozen at spawn, other character fine (the grants raise only the CURRENT character). **On an AP disc the Heart Tank handler never runs** (kind 0 is retargeted to the record stub), so Alia's Life Up is the only unclamped writer left — and there are exactly eight of them, ids 0-7 in bits 8-15 of the u32 at `0x800D1C80`. From 0.7.3 the client therefore stops 2 short of 0x7F for every Life Up not yet collected (`life_ceiling` in client.py), releasing each as it lands, so the illegal value is unreachable rather than repaired; the inline repair in the poll stays as the net. The engine-side cap is unfixed and the Life Up applier is NOT yet located. Also the client's "is a save resident" byte — see the `save_sane` note in client.py | ✅ |
| `0x800D1C48` | u8 | Max HP — Zero (per Ghidra; char-selected). **Per-character for real**: both the heart pickup handler and the DNA Life-Up applier raise only the CURRENT character, so X and Zero legitimately differ. The AP client raises both on purpose | ⚠️ |
| **PLAYER life bar frame index** — `0x80025D44` + `0x80026030` | — | **Disassembled + live-verified 2026-09-06.** Two identical copies pick the bar sprite as `0x88 + (maxHP − 32)/2`, clamped ONLY at the top (`slti ,153` → `0x98`). Block at each site: `lb v0,0x47(v1)` / `addiu v0,v0,-32` / `srl v1,v0,31` / `addu v0,v0,v1` / `sra v0,v0,1` / `addiu rD,v0,136`. **There is no lower clamp**, so a maximum below 32 indexes off the FRONT of the bar artwork and draws unrelated HUD sprites — the same defect X6 shipped and fixed, and X5's code is instruction-for-instruction identical. Frames `0x88..0x98` cover max HP 32..64. Above 64 the frame stops while the fill keeps going: the bar overflows its container, which is intended and deliberately left alone. `addiu v0,v0,-32` occurs **223 times** in the EXE, so a site is only identified by the whole block — an EXE-wide scan confirms there is no third copy. apworld floors the index at `0x88` via a 6-word hook at `0x80077890`, emitted only when `starting_hp < 32`; returns are `0x80025D54` / `0x80026040`. **Live A/B'd on a patched disc 2026-09-06** with `Scripts/mmx5_lifebar_sandbox.lua` | ✅ live 2026-09-06 |
...
| `0x800D1C0F` | s8 | **Stage-result byte — the gate on the whole stage-transition function `0x800205AC`.** Positive → the "stage finished" path, which carries the ACT ladder (leaving `0x10`/`0x11`/`0x12` sets ACT 6/7/8, i.e. Zero Space progress); 0 → `0x80020838`; negative → `0x80020794`. Nine writers, **none in the pause handler**, so an Exit Stage escape reaches the transition through an existing setter. **An escape stores 0** — measured four times (Zero Space 1 ×2, uncleared Squid Adler ×2), so the escape takes the ZERO path and the ACT ladder never runs: escaping Zero Space does NOT skip the stage or its check. The `0x80023BE8` guess (which would have made it positive) was wrong. Careful: the same byte has a second use as the boss-spawn "already beaten" flag (`0x8005702C`, overlay-findings:213). Full write-up in ghidra-findings §9.17 | ✅ escape value live-proven 2026-09-04 |
| `0x800D1C79` (writers) | u8 | **ACT is written from exactly three places**: memory-card load marshalling (`0x8001C94C`, `0x8001CC78` — field copies out of the save buffer) and the endgame ladder in `0x800205AC`. **The results overlay never writes it** (disc sector 24073 scanned: 5 reads at offset `0x79`, 0 stores). So a results sequence alone cannot advance Zero Space progress — only leaving a stage down the positive `0x1C0F` path can | ✅ static 2026-09-04 |
| `0x800D1C01` / `0x800D1C02` | u8, u8 | **Hub screen and phase.** `0x1C01` picks the screen through the table at `0x800F5354` (dispatcher `0x800F0C80`); `0x1C02` picks the phase within it (`0x800F10E0` → table `0x800F51B0` for screen 4). **Screen 4 phase 0 is the stage-select cursor** — the only code that writes `0x800D1C0C`, and the only place a blocked confirm can park a 0 there. Phases 1-7 are zoom → character select → load (**phase 7, `0x800F101C`, is where `0x1C0C` is finally READ**), 8-9 are the back-out. Full map in ghidra-findings §9.14. Any client write to `0x1C0C` must be gated on `(0x1C01, 0x1C02) == (4, 0)`: the hub overlay stays resident for the whole entry sequence, so the overlay anchor alone does NOT mean the player is still choosing | ✅ static 2026-09-03 |
