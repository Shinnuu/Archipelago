...
from worlds.mmx5.test import MMX5TestBase
...
# Player feedback was consistently "why do I have to edit a file I have never
# opened", so the YAML is the normal route now and host.yaml is an override
...
class TestTheYamlOptions(unittest.TestCase):
    def test_every_target_has_an_option(self):
        ...
        for target in palettes.TARGETS:
            self.assertIn(f"{target}_palette", MMX5Options.type_hints, target)

    def test_the_option_offers_vanilla_and_every_preset(self):
        from worlds.mmx5.options import XPalette
...
class TestOverridePrecedence(unittest.TestCase):
    ...
    def test_an_old_patch_still_takes_host_values(self):
        # A .apmmx5 generated before this change carries no colours at all.
...
class TestTheSeedCarriesTheColour(MMX5TestBase):
    options = {"x_palette": "emerald", "zero_palette": "gold"}

    def _carried(self):
        import json

        from worlds.mmx5.Rom import MMX5ProcedurePatch, patch_rom

        written = {}

        class _Capture(MMX5ProcedurePatch):
    ...
    def test_untouched_targets_are_vanilla(self):
        carried = self._carried()
        for target in ("falcon", "gaea", "ultimate"):
