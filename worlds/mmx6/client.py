...
class MMX6Client(BizHawkClient):
    ...
    def _grants(self, ctx: "BizHawkClientContext", save: bytes) -> list[tuple[int, bytes]]:
        ...
        #
        # 127 is the game's own ceiling - the gauge is read as a signed byte
        # everywhere and current HP is seven bits - so nothing above it is
        # ever written.
        #
        # ABOVE 64 THE BAR OVERFLOWS ITS FRAME, AND THAT IS ACCEPTED. The
        # frame is a sprite index with seventeen sizes covering 32..64
        # (0x8002497C) while the fill is drawn one notch per point of current
        # HP from a fixed anchor (0x80024F80), so past 64 the frame stops
        # growing and the fill runs on past its end. A 64 cap was tried on
        # 2026-09-03 and reverted the next day: the player who reported the
        # original bug saw the overflow in play and read it as correct, which
        # is how the genre usually draws it. The half that genuinely misdraws
        # is BELOW 32, where the index drops beneath the artwork entirely.
